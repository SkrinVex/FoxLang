...
`[...]`, а `==` и `copy` с ним справляются. Память такого кольца освобождается только
...
Структура — составной тип с именованными полями фиксированных типов. Объявляется на
верхнем уровне файла; у поля может быть значение по умолчанию.
...
| `json_decode(string json) -> any` | JSON целиком: объект — `map`, массив — `array`, целое — `int`, дробное — `float`, `null` — пустая строка |
