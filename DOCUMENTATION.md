...
* Через псевдоним доступны только функции и переменные, объявленные в самом модуле.
  Встроенные функции вроде `sqrt` вызываются без него: `m.sqrt(2)` — ошибка, и
  `foxlang check` подсказывает, как правильно.
...
* Псевдоним — обычная глобальная переменная (`type_of(m)` — `map`); имя не должно
  быть занято.
