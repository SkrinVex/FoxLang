...
не распаковываются на диск. Все аргументы после имени приложения достаются программе
через `os_args()` (или `args()` из `using os;`): `./app --help` передаёт ей `--help`.
Зарезервирован только `--foxlang-licenses` как единственный аргумент: он выводит
лицензии вместо запуска программы.
...
- Код возврата приложения — код программы: `0`, `1` при ошибке или значение `exit(code)`.
...
- HTTP(S)-клиент использует статический libcurl (версия зафиксирована в
  `cmake/Networking.cmake`), без дочерних процессов и внешнего `curl`. Linux
  статически включает Mbed TLS, Windows использует
...
- HTTP-сервер и TCP/DNS используют POSIX sockets на Linux и Winsock на Windows.
  ...
  Маршруты задаются для любого метода; обработчик читает query-строку и заголовки
  и может выбрать `Content-Type` ответа. Исключения обработчиков возвращают HTTP 500. Тайм-ауты чтения и записи — пять
...
GET/POST/PUT/DELETE с кодами статуса (включая 404 и отказ соединения), TCP echo, DNS,
webhook с Unicode по образцу Telegram, маршруты PUT/DELETE, query-строку, заголовки
и `Content-Type` ответа сервера.
